...
int test_program() {
    ...
    FILE *file_record;
    file_record = fopen(filename_clients, "r+");
    if (!file_record) {
        puts("Not accsess.");
        return ERROR;
    }
    ...
    file_transaction = fopen(filename_transactions, "r+");
    if (!file_transaction) {
        puts("Not accsess.");
        fclose(file_record);
        return ERROR;
    ...
    blackrecord = fopen(filename_res, "w+");
    if (!blackrecord) {
        puts("Not accsess.");
        fclose(file_record);
    ...
    Data expected_data = {1, "name_1", "surname_1", "address_1", "89201909927", 50, 110, 20};

    Data got_data = {0};

    rewind(blackrecord);

    if (fscanf(blackrecord, "%d", &got_data.number) != 1 || got_data.number != expected_data.number)
        rc = ERROR;

    if (fscanf(blackrecord, "%19s", got_data.name) != 1 || strcmp(got_data.name, expected_data.name) != 0)
        rc = ERROR;
    ...
    if (fscanf(blackrecord, "%14s", got_data.tel_number) != 1 ||
                strcmp(got_data.tel_number, expected_data.tel_number))
        rc = ERROR;

    if (fscanf(blackrecord, "%lf", &got_data.indebtedness) != 1 ||
        fabs(got_data.indebtedness - expected_data.indebtedness) >= EPS)
        rc = ERROR;

    if (fscanf(blackrecord, "%lf", &got_data.credit_limit) != 1 ||
        fabs(got_data.credit_limit - expected_data.credit_limit) >= EPS)
        rc = ERROR;

    if (fscanf(blackrecord, "%lf", &got_data.cash_payments) != 1 ||
        fabs(got_data.cash_payments - expected_data.cash_payments) >= EPS)
        rc = ERROR;
